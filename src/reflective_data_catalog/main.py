    ...
    def search(
        ...
        try:
            ...
            for entry_name in cat:
                ...
                metadata = entry._metadata if hasattr(entry, "_metadata") else {}
                tags = metadata.get("tags", [])
                drv = entry._driver if hasattr(entry, "_driver") else ""

                # --- term filter ---
                if term_lower is not None:
                    haystack = " ".join([entry_name, desc, drv, *tags]).lower()
                ...
                match_details.append((entry_name, "intake", desc[:80] if desc else ""))
        ...
        try:
            ...
            if term_lower is not None:
                ...
                mask = filtered[text_cols].apply(
                    lambda row: term_lower in " ".join(str(v) for v in row).lower(),
                    axis=1,
                )
                filtered = filtered[mask]

            if var_lower is not None and "variable_id" in filtered.columns:
                filtered = filtered[filtered["variable_id"].str.lower() == var_lower]
        ...
        for esgf_name, esgf_desc in esgf_entries:
            ...
            if var_lower is not None:
                ...
                if (
                    term_lower is not None
                    and term_lower not in (esgf_name + " " + esgf_desc).lower()
                ):
                    continue
