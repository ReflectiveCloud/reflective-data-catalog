...
- UKESM's old `time=` kwarg is removed on the Zarr-switched `ukesm1_ssp245` (CMOR tables) but retained on `ukesm1_g6_1p5k_hilla`, which is NetCDF-only under the `G6-1p5K-HiLLA` (1p5K spelling) prefix; `table='AMON'` / `variable='T'`-style NetCDF vocabulary changed with the Zarr switches — see `docs/migration-matrix.md`.
