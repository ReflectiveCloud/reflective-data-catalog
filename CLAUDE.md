...
- **Discovery** is delimiter-based: `_scan()` renders the urlpath with a marker at the target parameter, lists one level at that template depth via `CloudFileSystem.ls`, and extracts the observed values. Empty scans return `[]` with a warning — never the parameter defaults. Results are cached per source (pass `refresh=True` to rescan).
...
- UKESM's old `time=` kwarg is removed on the Zarr-switched `ukesm1_ssp245` (the time-frequency path level is gone; UM stream names ap4..onm carry over as `table=` values) but retained on `ukesm1_g6_1p5k_hilla`, which is NetCDF-only under the `G6-1p5K-HiLLA` (1p5K spelling) prefix; CESM/MIROC `table='AMON'`-style vocabulary changed with the Zarr switches, while E3SM keeps native variable names (`T`, `TREFHT` — no `tas`) — see `docs/migration-matrix.md`.
