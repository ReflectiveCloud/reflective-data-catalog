...
| `variable` | canonical; aliases: variable_id. On the grouped public-R2 Zarr entries (CESM/MIROC/simulator) variables are selected from the opened dataset, not a path parameter; list_variables() enumerates them from the store metadata, aggregating across the groups the call does not pin (e.g. list_variables(table='day')) |
